...
namespace QuantLib {
    ...
    class AdaptiveRungeKutta {
      public:
        typedef std::function<Disposable<std::vector<T> >(const Real, const std::vector<T>&)>
            OdeFct;
        ...
        */
        ...
        operator()(const OdeFct& ode, const std::vector<T>& y1, const Real x1, const Real x2);
        T operator()(const OdeFct1d& ode, const T y1, const Real x1, const Real x2);
    ...
    template <class T>
    Disposable<std::vector<T> > AdaptiveRungeKutta<T>::operator()(const OdeFct& ode,
    ...
    namespace detail {
        ...
        struct OdeFctWrapper {
            ...
            OdeFctWrapper(const OdeFct1d& ode1d) : ode1d_(ode1d) {}
            Disposable<std::vector<T> > operator()(const Real x, const std::vector<T>& y) {
