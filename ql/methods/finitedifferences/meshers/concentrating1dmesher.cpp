...
namespace QuantLib {
    ...
    Concentrating1dMesher::Concentrating1dMesher(Real start,
                                                 Real end,
                                                 Size size,
    ...
    : Fdm1dMesher(size) {
        ...
        const Real density =
            cPoints.second == Null<Real>() ? Null<Real>() : cPoints.second * (end - start);
        ...
        if (cPoint != Null<Real>()) {
            ...
            if (requireCPoint) {
                ...
                if (!close(cPoint, start) && !close(cPoint, end)) {
                    ...
                    const Real u0 = std::max(std::min(static_cast<int>(z0 * (size - 1) + 0.5),
    ...
    : Fdm1dMesher(size) {
        ...
        for (const auto& cPoint : cPoints) {
            ...
            betas.push_back(square<Real>()(std::get<1>(cPoint) * (end - start)));
        }
        ...
        const Real a = Brent().solve([&](Real x) { return fct.solve(x, start, 0.0, 1.0) - end; },
                                     tol, aInit, 0.1 * aInit);
        ...
        for (Size i = 0; i < points.size(); ++i) {
            if (std::get<2>(cPoints[i]) && points[i] > start && points[i] < end) {
                ...
                const Size j =
                    std::distance(y.begin(), std::lower_bound(y.begin(), y.end(), points[i]));

                const Real e =
                    Brent().solve([&](Real x) { return odeSolution(x, true) - points[i]; },
