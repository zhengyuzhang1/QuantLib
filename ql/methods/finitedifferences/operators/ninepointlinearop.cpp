...
namespace QuantLib {
    NinePointLinearOp::NinePointLinearOp(Size d0,
                                         Size d1,
                                         const std::shared_ptr<FdmMesher>& mesher,
                                         Size mesher_size)
    : d0_(d0), d1_(d1), i00_(new Size[mesher_size]), i10_(new Size[mesher_size]),
      i20_(new Size[mesher_size]), i01_(new Size[mesher_size]), i21_(new Size[mesher_size]),
      i02_(new Size[mesher_size]), i12_(new Size[mesher_size]), i22_(new Size[mesher_size]),
      a00_(new Real[mesher_size]), a10_(new Real[mesher_size]), a20_(new Real[mesher_size]),
      a01_(new Real[mesher_size]), a11_(new Real[mesher_size]), a21_(new Real[mesher_size]),
      a02_(new Real[mesher_size]), a12_(new Real[mesher_size]), a22_(new Real[mesher_size]),
      mesher_(mesher) {}

    NinePointLinearOp::NinePointLinearOp(Size d0, Size d1, const std::shared_ptr<FdmMesher>& mesher)
    : NinePointLinearOp(d0, d1, mesher, mesher->layout()->size()) {

        QL_REQUIRE(d0_ != d1_ && d0_ < mesher->layout()->dim().size() &&
    ...
    : NinePointLinearOp(m.d0_, m.d1_, m.mesher_, m.mesher_->layout()->size()) {
    ...
    NinePointLinearOp& NinePointLinearOp::operator=(const Disposable<NinePointLinearOp>& m) {
        swap(const_cast<Disposable<NinePointLinearOp>&>(m));
        return *this;
    }

    NinePointLinearOp::NinePointLinearOp(const Disposable<NinePointLinearOp>& from) {
        swap(const_cast<Disposable<NinePointLinearOp>&>(from));
    }

    Disposable<Array> NinePointLinearOp::apply(const Array& u) const {

        const std::shared_ptr<FdmLinearOpLayout> index = mesher_->layout();
        QL_REQUIRE(u.size() == index->size(),
                   "inconsistent length of r " << u.size() << " vs " << index->size());
        ...
        for (Size i = 0; i < retVal.size(); ++i) {
            retVal[i] = a00[i] * u[i00[i]] + a01[i] * u[i01[i]] + a02[i] * u[i02[i]] +
                        a10[i] * u[i10[i]] + a11[i] * u[i] + a12[i] * u[i12[i]] +
                        a20[i] * u[i20[i]] + a21[i] * u[i21[i]] + a22[i] * u[i22[i]];
        }
        return retVal;
    }
    ...
    Disposable<NinePointLinearOp> NinePointLinearOp::mult(const Array& u) const {
