    ...
    */
    ...
    public:
        RiskyFixedBond(const std::string& name,
                       ...
                       Natural settlementDays = 0);
        std::vector<std::shared_ptr<CashFlow> > cashflows() const override;
        Real notional(Date date = Date::minDate()) const override;
        Date effectiveDate() const override;
        Date maturityDate() const override;
        std::vector<std::shared_ptr<CashFlow> > interestFlows() const override;
        std::vector<std::shared_ptr<CashFlow> > notionalFlows() const override;
    private:
        Schedule schedule_;
        Real rate_;
