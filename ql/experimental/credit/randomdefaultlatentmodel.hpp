...
namespace QuantLib {
    ...
    */
    ...
    class RandomLM : public virtual LazyObject, public virtual DefaultLossModel {
      ...
      protected:
        ...
        void performCalculations() const override {
            static_cast<const derivedRandomLM<copulaPolicy, USNG>*>(this)->initDates(); // in
                                                                                        // update?
        ...
        */
        ...
        virtual std::tuple<Real, Real, Real> percentileAndInterval(const Date& d,
        ...
        */
        ...
        /*! Distributes the total VaR amount along the portfolio
            ...
            The passed loss amount is in loss units.
        */
        virtual Disposable<std::vector<std::vector<Real> > >
    ...
    RandomLM<D, C, URNG>::probsBeingNthEvent(Size n, const Date& d) const {
        calculate();
        ...
        for (Size iSim = 0; iSim < nSims_; iSim++) {
            ...
            if (namesDefaulting.size() >= n) {
                std::map<unsigned short, unsigned short>::const_iterator itdefs =
                    namesDefaulting.begin();
        ...
        std::transform(hitsByDate.begin(), hitsByDate.end(), hitsByDate.begin(),
    ...
    template <template <class, class> class D, class C, class URNG>
    std::pair<Real, Real>
    RandomLM<D, C, URNG>::expectedTrancheLossInterval(const Date& d,
                                                      Probability confidencePerc) const {
        calculate();
        Date today = Settings::instance().evaluationDate();
        ...
        return std::make_pair(lossStats.mean(),
                              lossStats.errorEstimate() * InverseCumulativeNormal::standard_value(
    ...
    Real RandomLM<D, C, URNG>::expectedShortfall(const Date& d, Real percent) const {
        ...
        Probability probOverQ =
            static_cast<Real>(std::distance(losses.begin() + position, losses.end())) /
    ...
    */
    ...
    RandomLM<D, C, URNG>::percentileAndInterval(const Date& d, Real percentile) const {
        ...
        for (Size delta = 1; delta < quantilePosition; delta++) {
            Real cached =
                incompleteBetaFunction(Real(s), Real(nSims_ + 1 - s), percentile, 1.e-8, 500);
            Real pMinus =
                ...
                by '-1'*/
                incompleteBetaFunction(Real(r + 1), Real(nSims_ - r), percentile, 1.e-8, 500) -
                cached;
            Real pPlus =
                incompleteBetaFunction(Real(r), Real(nSims_ - r + 1), percentile, 1.e-8, 500) -
                cached;
        ...
        return std::tuple<Real, Real, Real>(quantileValue, lowerPercentile, upperPercentile);
    }


    template <template <class, class> class D, class C, class URNG>
    Disposable<std::vector<Real> > RandomLM<D, C, URNG>::splitVaRLevel(const Date& date,
                                                                       Real loss) const {
    ...
    requested level.*/
    Disposable<std::vector<std::vector<Real> > > RandomLM<D, C, URNG>::splitVaRAndError(
        ...
        for (Size iSim = 0; iSim < nSims_; iSim++) {
            ...
            if (portfSimLoss > loss) {
                ...
                for (Size i = 0; i < splitEventsBuffer.size(); i++) {
                    ...
                    Real tranchedLossBefore = std::min(std::max(ptflCumulLoss - attachAmount, 0.),
                                                       detachAmount - attachAmount);
                    ptflCumulLoss += lossName;
                    Real tranchedLossAfter = std::min(std::max(ptflCumulLoss - attachAmount, 0.),
                                                      detachAmount - attachAmount);
                    // assign new losses:
        ...
        class Root {
          public:
            ...
            Real operator()(Real t) const {
                ...
                read only)
                */
                return dts_->defaultProbability(curveRef_ + Period(static_cast<Integer>(t), Days),
                                                true) -
                       pd_;
            }
    ...
    */
    ...
    class RandomDefaultLM : public RandomLM<RandomDefaultLM, copulaPolicy, USNG> {
      ...
      public:
        ...
        RandomDefaultLM(const std::shared_ptr<DefaultLatentModel<copulaPolicy> >& model,
        ...
        : RandomLM< ::QuantLib::RandomDefaultLM, copulaPolicy, USNG>(
          ...
          model_(model),
          recoveries_(recoveries.size() == 0 ? std::vector<Real>(model->size(), 0.) : recoveries),
          accuracy_(accuracy) {
            // redundant through basket?
            this->registerWith(Settings::instance().evaluationDate());
            this->registerWith(model_);
        }
        RandomDefaultLM(const std::shared_ptr<ConstantLossLatentmodel<copulaPolicy> >& model,
                        Size nSims = 0, // stats will crash on div by zero, FIX ME.
                        Real accuracy = 1.e-6,
                        BigNatural seed = 2863311530UL)
        : RandomLM< ::QuantLib::RandomDefaultLM, copulaPolicy, USNG>(
