...
namespace QuantLib {
    ...
    namespace {
        std::shared_ptr<Fdm1dMesher> varianceMesher(const SquareRootProcessRNDCalculator& rnd,
                                                    Time t0,
                                                    Time t1,
                                                    Size vGrid,
                                                    Real v0,
            ...
            switch (params.trafoType) {
                case FdmSquareRootFwdOp::Log: {
                    ...
                    return std::make_shared<Concentrating1dMesher>(lowerBound, upperBound, vGrid,
                                                                   cPoints, 1e-8);
                } break;
        ...
        Real integratePDF(const Array& p,
            ...
            if (trafoType != FdmSquareRootFwdOp::Power) {
                ...
                for (FdmLinearOpIterator iter = mesher->layout()->begin(); iter != end; ++iter) {
        ...
        Disposable<Array> rescalePDF(const Array& p,
                                     const std::shared_ptr<FdmMesherComposite>& mesher,
                                     FdmSquareRootFwdOp::TransformationType trafoType,
                                     Real alpha) {
        ...
        Disposable<Array> reshapePDF(const Array& p,
            ...
            const std::shared_ptr<FdmLinearOpLayout> oldLayout = oldMesher->layout();
            const std::shared_ptr<FdmLinearOpLayout> newLayout = newMesher->layout();
            ...
            for (Size i = 0; i < m.rows(); ++i) {
                std::copy(p.begin() + i * m.columns(), p.begin() + (i + 1) * m.columns(),
                          m.row_begin(i));
            }
            const Interpolation2D interpol =
            ...
            for (FdmLinearOpIterator iter = newLayout->begin(); iter != endIter; ++iter) {
                const Real x = newMesher->location(iter, 0);
                const Real v = newMesher->location(iter, 1);

                if (x > interpol.xMax() || x < interpol.xMin() || v > interpol.yMax() ||
                    v < interpol.yMin()) {
                    pNew[iter.index()] = 0;
                } else {
        ...
        std::shared_ptr<FdmScheme>
        fdmSchemeFactory(const FdmSchemeDesc desc,
                         const std::shared_ptr<FdmLinearOpComposite>& op) {

            switch (desc.type) {
                case FdmSchemeDesc::HundsdorferType:
                    return std::shared_ptr<FdmScheme>(new FdmSchemeWrapper<HundsdorferScheme>(
                ...
                case FdmSchemeDesc::CraigSneydType:
                    return std::shared_ptr<FdmScheme>(new FdmSchemeWrapper<CraigSneydScheme>(
    ...
    : localVol_(std::move(localVol)), hestonModel_(std::move(hestonModel)), endDate_(endDate),
      params_(std::move(params)), mandatoryDates_(std::move(mandatoryDates)), logging_(logging) {
    ...
    std::shared_ptr<LocalVolTermStructure> HestonSLVFDMModel::leverageFunction() const {
        calculate();
    ...
    void HestonSLVFDMModel::performCalculations() const {
        ...
        const std::shared_ptr<HestonProcess> hestonProcess = hestonModel_->process();
        const std::shared_ptr<Quote> spot = hestonProcess->s0().currentLink();
        const std::shared_ptr<YieldTermStructure> rTS = hestonProcess->riskFreeRate().currentLink();
        ...
        const std::shared_ptr<TimeGrid> timeGrid(new TimeGrid(times.begin(), times.end()));

        // build 1d meshers
        const LocalVolRNDCalculator localVolRND(spot, rTS, qTS, localVol_.currentLink(), timeGrid,
                                                xGrid, params_.x0Density, params_.localVolEpsProb,
                                                params_.maxIntegrationIterations);

        const std::vector<Size> rescaleSteps = localVolRND.rescaleTimeSteps();

        const SquareRootProcessRNDCalculator squareRootRnd(v0, kappa, theta, sigma);
        ...
        vMesher.emplace_back(std::make_shared<Predefined1dMesher>(std::vector<Real>(vGrid, v0)));
        ...
        for (Size i = 1; i < timeGrid->size(); ++i) {
            ...
            if ((rescaleIdx < rescaleSteps.size()) && (i == rescaleSteps[rescaleIdx])) {
                ...
                vMesher.push_back(varianceMesher(
                    ...
                    (rescaleIdx < rescaleSteps.size()) ? timeGrid->at(rescaleSteps[rescaleIdx]) :
                                                         timeGrid->back(),
                    vGrid, v0, params_));
            } else
        ...
        std::shared_ptr<FdmMesherComposite> mesher =
            std::make_shared<FdmMesherComposite>(xMesher.at(1), vMesher.at(1));
        ...
        std::vector<std::shared_ptr<std::vector<Real> > > vStrikes(timeGrid->size());
        ...
        for (Size i = 0; i < timeGrid->size(); ++i) {
            ...
            if (xMesher[i]->locations().front() == xMesher[i]->locations().back()) {
                ...
                std::transform(xMesher[i]->locations().begin(), xMesher[i]->locations().end(),
                               vStrikes[i]->begin(), static_cast<Real (*)(Real)>(std::exp));
            }
        }
        ...
        if (logging_) {
            const LogEntry entry = {timeGrid->at(1), std::make_shared<Array>(p), mesher};
        ...
        for (Size i = 2; i < times.size(); ++i) {
            ...
            for (Size r = 0; r < params_.predictionCorretionSteps; ++r) {
                const FdmSchemeDesc fdmSchemeDesc = (i < params_.nRannacherTimeSteps + 2) ?
                ...
                for (Size j = 0; j < x.size(); ++j) {
                    ...
                    const Real l = (scale >= 0.0) ? localVol * std::sqrt(scale) : 1.0;
                ...
                const Real sLowerBound = std::max(
                    x.front(), std::exp(localVolRND.invcdf(params_.leverageFctPropEps, t)));
                const Real sUpperBound = std::min(
                    x.back(), std::exp(localVolRND.invcdf(1.0 - params_.leverageFctPropEps, t)));
                ...
                for (Size j = 0; j < x.size(); ++j) {
                    if (x[j] < sLowerBound)
                        std::fill(L->row_begin(j) + i,
                                  std::min(L->row_begin(j) + i + 1, L->row_end(j)), lowerL);
                    else if (x[j] > sUpperBound)
                        std::fill(L->row_begin(j) + i,
                                  std::min(L->row_begin(j) + i + 1, L->row_end(j)), upperL);
            ...
            if (logging_) {
                const LogEntry entry = {t, std::make_shared<Array>(p), mesher};
                logEntries_.push_back(entry);
            }
        }
