...
namespace QuantLib {
    ...
    : localVol_(std::move(localVol)), hestonModel_(std::move(hestonModel)),
      ...
      nBins_(nBins), calibrationPaths_(calibrationPaths) {
        ...
        timeGrid_ = std::make_shared<TimeGrid>(
            gridTimes.begin(), gridTimes.end(),
    ...
    std::shared_ptr<LocalVolTermStructure> HestonSLVMCModel::leverageFunction() const {
        calculate();
    ...
    void HestonSLVMCModel::performCalculations() const {
        ...
        std::vector<std::shared_ptr<std::vector<Real> > > vStrikes(timeGrid_->size());
        for (auto& vstrike : vStrikes) {

            const Integer u = nBins_ / 2;
            const Real dx = spot->value() * std::sqrt(QL_EPSILON);
            vstrike = std::make_shared<std::vector<Real> >(nBins_);

            for (Integer j = 0; j < Integer(nBins_); ++j)
                vstrike->at(j) = spot->value() + (j - u) * dx;
        }

        std::fill(L->column_begin(0), L->column_end(0), lv0);

        leverageFunction_ = std::make_shared<FixedLocalVolSurface>(
            referenceDate, std::vector<Time>(timeGrid_->begin(), timeGrid_->end()), vStrikes, L,
            dc);

        const std::shared_ptr<HestonSLVProcess> slvProcess =
            std::make_shared<HestonSLVProcess>(hestonProcess, leverageFunction_);

        std::vector<std::pair<Real, Real> > pairs(calibrationPaths_,
                                                  std::make_pair(spot->value(), v0));
        ...
        for (Size n = 1; n < timeGrid_->size(); ++n) {
            ...
            for (Size i = 0; i < nBins_; ++i) {
                ...
                (*L)[i][n] = std::sqrt(
                    square<Real>()(localVol_->localVol(t, vStrikes[n]->at(i), true)) / sum);
