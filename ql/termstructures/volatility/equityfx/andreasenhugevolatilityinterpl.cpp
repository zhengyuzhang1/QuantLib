...
namespace QuantLib {
    ...
    class AndreasenHugeCostFunction : public CostFunction {
      public:
        ...
        Disposable<Array> d2CdK2(const Array& c) const { return d2CdK2_.apply(c); }

        Disposable<Array> solveFor(Time dT, const Array& sig, const Array& b) const {
            ...
            switch (interpolationType_) {
                case AndreasenHugeVolatilityInterpl::CubicSpline:
                    sigInterpl = CubicNaturalSpline(lnMarketStrikes_.begin(),
                                                    lnMarketStrikes_.end(), sig.begin());
                    break;
                case AndreasenHugeVolatilityInterpl::Linear:
                    sigInterpl = LinearInterpolation(lnMarketStrikes_.begin(),
                                                     lnMarketStrikes_.end(), sig.begin());
                    break;
            ...
            for (FdmLinearOpIterator iter = layout->begin(); iter != endIter; ++iter) {
                ...
                const Real vol = sigInterpl(
                    std::min(std::max(lnStrike, lnMarketStrikes_.front()), lnMarketStrikes_.back()),
                    true);
        ...
        Disposable<Array> apply(const Array& c) const { return -mapT_.apply(c); }
        ...
        Disposable<Array> values(const Array& sig) const override {
            ...
            const MonotonicCubicNaturalSpline interpl(gridPoints.begin(), gridPoints.end(),
                                                      newNPVs.begin());
    ...
    : spot_(std::move(spot)), rTS_(std::move(rTS)), qTS_(std::move(qTS)),
      ...
      optimizationMethod_(std::move(optimizationMethod)), endCriteria_(endCriteria) {
        ...
        for (Size i = 0; i < calibrationSet.size(); ++i) {
            ...
            const Size l = std::distance(
                expiries.begin(), std::lower_bound(expiries.begin(), expiries.end(), expiry));

            const Real strike =
                std::dynamic_pointer_cast<PlainVanillaPayoff>(calibrationSet[i].first->payoff())
                    ->strike();

            const Size k =
                std::distance(strikes_.begin(), std::find_if(strikes_.begin(), strikes_.end(),
    ...
    std::shared_ptr<AndreasenHugeCostFunction> AndreasenHugeVolatilityInterpl::buildCostFunction(
        ...
        const Size nOptions = std::count_if(calibrationMatrix_[iExpiry].begin(),
                                            calibrationMatrix_[iExpiry].end(), not_null<Size>());
        ...
        return std::make_shared<AndreasenHugeCostFunction>(marketNPVs, marketVegas, lnMarketStrikes,
                                                           previousNPVs, mesher_, dT_[iExpiry],
                                                           interpolationType_);
    }
    ...
    void AndreasenHugeVolatilityInterpl::performCalculations() const {
        ...
        mesher_ = std::make_shared<FdmMesherComposite>(std::make_shared<Concentrating1dMesher>(
            std::log(minStrike() / spot_->value()), std::log(maxStrike() / spot_->value()),
            nGridPoints_, std::pair<Real, Real>(0.0, 0.025)));
        ...
        for (Size i = 0; i < expiries_.size(); ++i) {
            ...
            switch (calibrationType_) {
                case CallPut: {
                    ...
                    const Real fwd = spot_->value() * qTS_->discount(expiryTimes_[i]) /
                                     rTS_->discount(expiryTimes_[i]);
            ...
            avgError_ += std::accumulate(vegaDiffs.begin(), vegaDiffs.end(), 0.0);
    ...
    Date AndreasenHugeVolatilityInterpl::maxDate() const { return expiries_.back(); }
    ...
    Size AndreasenHugeVolatilityInterpl::getExerciseTimeIdx(Time t) const {
        return std::min<Size>(
            ...
            std::distance(expiryTimes_.begin(),
                          std::upper_bound(expiryTimes_.begin(), expiryTimes_.end(), t)));
    }
    ...
    Disposable<Array> AndreasenHugeVolatilityInterpl::getPriceSlice(Time t,
    ...
    Real AndreasenHugeVolatilityInterpl::optionPrice(Time t,
                                                     Real strike,
                                                     Option::Type optionType) const {
        ...
        const std::shared_ptr<Array> prices(std::make_shared<Array>(gridPoints_));
        ...
        priceCache_[t] = std::make_tuple(
            fwd, prices,
            std::make_shared<CubicNaturalSpline>(gridPoints_.begin() + 1, gridPoints_.end() - 1,
                                                 prices->begin() + 1));
    ...
    Disposable<Array>
    AndreasenHugeVolatilityInterpl::getLocalVolSlice(Time t, Option::Type optionType) const {

        const Size iu = getExerciseTimeIdx(t);

        const Array& previousNPVs = (optionType == Option::Call) ?
                                        calibrationResults_[iu].callNPVs :
                                        calibrationResults_[iu].putNPVs;
        ...
        const Array dCdT = costFunction->solveFor(
            dt, sig, costFunction->apply(costFunction->solveFor(dt, sig, previousNPVs)));
    ...
    Volatility AndreasenHugeVolatilityInterpl::localVol(Time t, Real strike) const {
        ...
        const std::shared_ptr<Array> localVol(std::make_shared<Array>(gridPoints_.size()));
        ...
        localVolCache_[t] = std::make_tuple(
            fwd, localVol,
            std::make_shared<LinearInterpolation>(gridPoints_.begin() + 1, gridPoints_.end() - 1,
                                                  localVol->begin() + 1));
