...
void HybridHestonHullWhiteProcessTest::testBsmHullWhiteEngine() {
    ...
    const Real tol = 1e-8;
    std::vector<Real> corr = {-0.75, -0.25, 0.0, 0.25, 0.75 };
    std::vector<Volatility> expectedVol = { 0.217064577, 0.243995801,
                                            0.256402830, 0.268236596, 0.290461343 };

    for (Size i=0; i < corr.size(); ++i) {
    ...
    for (double i : corr) {
        for (double j : strike) {
            ...
            std::shared_ptr<StrikedTypePayoff> payoff(
                               new PlainVanillaPayoff(Option::Put, j));
            std::shared_ptr<Exercise> exercise(
            ...
            optionBsmHW.setPricingEngine(
                std::shared_ptr<PricingEngine>(
                           new AnalyticBSMHullWhiteEngine(i, bsmProcess,
                                                          hwModel)));
        ...
        for (double j : strike) {
            std::shared_ptr<HybridHestonHullWhiteProcess> jointProcess(
                new HybridHestonHullWhiteProcess(
                        ...
                        i, HybridHestonHullWhiteProcess::Euler));
...
void HybridHestonHullWhiteProcessTest::testCallableEquityPricing() {
    ...
    */
    ...
    std::transform(schedule.begin(), schedule.end(), times.begin(),
                   std::bind(&Actual365Fixed::yearFraction,
...
void HybridHestonHullWhiteProcessTest::testFdmHestonHullWhiteEngine() {
    ...
    for (double i : corr) {
        for (double j : strike) {
            std::shared_ptr<StrikedTypePayoff> payoff(
                              new PlainVanillaPayoff(Option::Call, j));
            ...
            option.setPricingEngine(std::shared_ptr<PricingEngine>(
                new AnalyticBSMHullWhiteEngine(i,
...
void HybridHestonHullWhiteProcessTest::testH1HWPricingEngine() {
    ...
    */
    ...
    std::vector<Real> sigma_v = { 0.3, 0.6 };
    ...
    std::vector<Real> strikes = {40, 80, 100, 120, 180 };
