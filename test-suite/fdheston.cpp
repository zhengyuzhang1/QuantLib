...
namespace {
    ...
    class ParableLocalVolatility : public LocalVolTermStructure {
      public:
        ParableLocalVolatility(const Date& referenceDate,
                               Real s0,
                               Real alpha,
                               const DayCounter& dayCounter)
        : LocalVolTermStructure(referenceDate, NullCalendar(), Following, dayCounter),
          referenceDate_(referenceDate), s0_(s0), alpha_(alpha) {}
...
void FdHestonTest::testFdmHestonVarianceMesher() {
    ...
    const std::shared_ptr<HestonProcess> process(std::make_shared<HestonProcess>(
        ...
        Handle<YieldTermStructure>(flatRate(0.02, dc)),
        Handle<Quote>(std::make_shared<SimpleQuote>(100.0)), 0.09, 1.0, 0.09, 0.2, -0.5));
    ...
    const std::shared_ptr<FdmHestonLocalVolatilityVarianceMesher> constSlvMesher =
        std::make_shared<FdmHestonLocalVolatilityVarianceMesher>(5, process, lVol, 1.0);
    ...
    const std::shared_ptr<FdmHestonLocalVolatilityVarianceMesher> slvMesher =
        std::make_shared<FdmHestonLocalVolatilityVarianceMesher>(5, process, leverageFct, 0.5, 1,
                                                                 0.01);

    const Real initialVolEstimate =
        std::make_shared<FdmHestonVarianceMesher>(5, process, 0.5, 1, 0.01)->volaEstimate();
    ...
    if (std::fabs(volaEstExpected - volaEstCalculated) > 0.001) {
        BOOST_ERROR("Failed to reproduce Heston local volatility "
                    ...
                    << "\n    calculated: " << calculatedVol << "\n    expected:   " << expectedVol
                    << std::scientific << "\n    difference  "
                    << std::fabs(volaEstExpected - volaEstCalculated)
                    << "\n    tolerance:  " << tol);
    }
}
...
void FdHestonTest::testFdmHestonBarrierVsBlackScholes() {
    ...
    Handle<Quote> spot(std::shared_ptr<Quote>(new SimpleQuote(0.0)));
    ...
    std::shared_ptr<PricingEngine> analyticEngine(new AnalyticBarrierEngine(bsProcess));
    ...
    for (auto& value : values) {
        ...
        barrierOption.setPricingEngine(std::shared_ptr<PricingEngine>(
            new FdHestonBarrierEngine(std::make_shared<HestonModel>(hestonProcess), 200, 101, 3)));
...
void FdHestonTest::testFdmHestonBarrier() {
    ...
    std::shared_ptr<StrikedTypePayoff> payoff(new PlainVanillaPayoff(Option::Call, 100));

    BarrierOption barrierOption(Barrier::UpOut, 135, 0.0, payoff, exercise);

    barrierOption.setPricingEngine(std::shared_ptr<PricingEngine>(
        new FdHestonBarrierEngine(std::make_shared<HestonModel>(hestonProcess), 50, 400, 100)));
...
void FdHestonTest::testFdmHestonAmerican() {
    ...
    std::shared_ptr<StrikedTypePayoff> payoff(new PlainVanillaPayoff(Option::Put, 100));

    VanillaOption option(payoff, exercise);
    std::shared_ptr<PricingEngine> engine(
        new FdHestonVanillaEngine(std::make_shared<HestonModel>(hestonProcess), 200, 100, 50));
...
void FdHestonTest::testFdmHestonIkonenToivanen() {
    ...
    */
    ...
    std::shared_ptr<StrikedTypePayoff> payoff(new PlainVanillaPayoff(Option::Put, 10));

    VanillaOption option(payoff, exercise);

    std::vector<Real> strikes = {8, 9, 10, 11, 12};
    std::vector<Real> expected = {2.00000, 1.10763, 0.520038, 0.213681, 0.082046};
    ...
    for (Size i = 0; i < strikes.size(); ++i) {
        ...
        std::shared_ptr<PricingEngine> engine(
            new FdHestonVanillaEngine(std::make_shared<HestonModel>(hestonProcess), 100, 400));
        option.setPricingEngine(engine);
...
void FdHestonTest::testFdmHestonBlackScholes() {
    ...
    for (double strike : strikes) {
        ...
        option.setPricingEngine(
            std::shared_ptr<PricingEngine>(new AnalyticEuropeanEngine(bsProcess)));
        ...
        option.setPricingEngine(std::shared_ptr<PricingEngine>(
            new FdHestonVanillaEngine(std::make_shared<HestonModel>(hestonProcess), 100, 400, 3)));
        ...
        // Explicit scheme
        option.setPricingEngine(std::shared_ptr<PricingEngine>(
            new FdHestonVanillaEngine(std::make_shared<HestonModel>(hestonProcess), 4000, 400, 3, 0,
...
void FdHestonTest::testFdmHestonEuropeanWithDividends() {
    ...
    std::shared_ptr<PricingEngine> engine(
        new FdHestonVanillaEngine(std::make_shared<HestonModel>(hestonProcess), 50, 100, 50));
    option.setPricingEngine(engine);
        ...
        for (auto& value : values) {
            for (unsigned long j : tn) {
                for (double k : v0) {
                    ...
                    std::shared_ptr<PricingEngine> analyticEngine(new AnalyticHestonEngine(
...
void FdHestonTest::testFdmHestonIntradayPricing() {
    ...
    const std::shared_ptr<Exercise> europeanExercise(new EuropeanExercise(maturity));
    const std::shared_ptr<StrikedTypePayoff> payoff(new PlainVanillaPayoff(type, strike));
    VanillaOption option(payoff, europeanExercise);

    const Handle<Quote> s0(std::shared_ptr<Quote>(new SimpleQuote(underlying)));
    ...
    const std::shared_ptr<PricingEngine> fdm(new FdHestonVanillaEngine(model, 20, 100, 26, 0));
    ...
    for (Size i = 0; i < 10; ++i) {
        ...
        flatTermStructure.linkTo(
            std::shared_ptr<YieldTermStructure>(new FlatForward(now, riskFreeRate, dayCounter)));
        flatDividendTS.linkTo(
            std::shared_ptr<YieldTermStructure>(new FlatForward(now, dividendYield, dayCounter)));
...
void FdHestonTest::testMethodOfLinesAndCN() {
    ...
    const std::shared_ptr<HestonModel> model(std::make_shared<HestonModel>(
    ...
    const std::shared_ptr<PricingEngine> fdmMol(std::make_shared<FdHestonVanillaEngine>(
    ...
    VanillaOption option(payoff, std::make_shared<AmericanExercise>(maturity));
    ...
    const std::shared_ptr<PricingEngine> fdmCN(std::make_shared<FdHestonVanillaEngine>(
    ...
    BarrierOption barrierOption(Barrier::DownOut, 85.0, 10.0, payoff,
                                std::make_shared<EuropeanExercise>(maturity));

    barrierOption.setPricingEngine(std::make_shared<FdHestonBarrierEngine>(model, 100, 31, 11));

    const Real expectedBarrier = barrierOption.NPV();

    barrierOption.setPricingEngine(std::make_shared<FdHestonBarrierEngine>(
        model, 100, 31, 11, 0, FdmSchemeDesc::MethodOfLines()));
    ...
    barrierOption.setPricingEngine(std::make_shared<FdHestonBarrierEngine>(
        model, 100, 31, 11, 0, FdmSchemeDesc::CrankNicolson()));
...
void FdHestonTest::testSpuriousOscillations() {
    ...
    const std::shared_ptr<FdHestonVanillaEngine> hestonEngine(
        std::make_shared<FdHestonVanillaEngine>(model, 6, 200, 13, 0, FdmSchemeDesc::TrBDF2()));

    VanillaOption option(std::make_shared<PlainVanillaPayoff>(Option::Call, spot->value()),
                         std::make_shared<EuropeanExercise>(maturity));
    ...
    const std::tuple<FdmSchemeDesc, std::string, bool> descs[] = {
        ...
        std::make_tuple(FdmSchemeDesc::ModifiedHundsdorfer(), "Mod. Hundsdorfer", true),
