...
void DoubleBinaryOptionTest::testHaugValues() {
    ...
    for (auto & value : values) {
        ...
        ext::shared_ptr<StrikedTypePayoff> payoff(new CashOrNothingPayoff(
            Option::Call, 0, value.cash));

        Date exDate = today + Integer(value.t*360+0.5);
        ext::shared_ptr<Exercise> exercise;
        if (value.barrierType == DoubleBarrier::KIKO ||
        ...
        DoubleBarrierOption opt(value.barrierType, 
                          value.barrier_lo, 
                          value.barrier_hi, 
                          0,
                          payoff,
        ...
        if (error > value.tol) {
            REPORT_FAILURE("value", payoff, exercise, value.barrierType, 
                           value.barrier_lo, value.barrier_hi, value.s,
        ...
        double tol = 0.22;
        if (error>tol) {
            REPORT_FAILURE("Binomial value", payoff, exercise, value.barrierType, 
                           value.barrier_lo, value.barrier_hi, value.s,
                           value.q, value.r, today, value.v,
