...
void HestonModelTest::testBlackCalibration() {
    ...
    for (auto & optionMaturitie : optionMaturities) {
        ...
        options.emplace_back(std::make_shared<HestonModelHelper>(optionMaturitie, calendar,
                                                s0, strikePrice, vol,
...
void HestonModelTest::testDAXCalibration() {
    ...
    const std::vector<std::shared_ptr<BlackCalibrationHelper> > options
                                                    = marketData.options;
...
void HestonModelTest::testFdVanillaVsCached() {
    ...
    payoff = std::make_shared<PlainVanillaPayoff>(Option::Call, 95.0);
    ...
    process = std::make_shared<HestonProcess>(
                   riskFreeTS, dividendTS, s0, 0.04, 1.0, 0.04, 0.001, 0.0);
    payoff = std::make_shared<PlainVanillaPayoff>(Option::Put, 95.0);
...
void HestonModelTest::testAlanLewisReferencePrices() {
    ...
    std::vector<std::shared_ptr<PricingEngine>> engines
    ...
    for (Size j=0; j < terms.size(); ++j) {
        ...
        const std::shared_ptr<HestonExpansion> forde =
            std::make_shared<FordeHestonExpansion>(kappa, theta, sigma,
                                                     v0, rho, term);
        std::vector<std::shared_ptr<HestonExpansion>> expansions = { lpp2, lpp3, forde };
        for (Size i=0; i < strikes.size(); ++i) {
            const Real strike = strikes[i];
...
void HestonModelTest::testAndersenPiterbargPricing() {
    ...
    std::vector<std::shared_ptr<AnalyticHestonEngine>> engines = {
        andersenPiterbargLaguerreEngine,
        andersenPiterbargLobattoEngine,
        andersenPiterbargSimpsonEngine,
        andersenPiterbargTrapezoidEngine,
