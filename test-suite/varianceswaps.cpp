...
void VarianceSwapTest::testReplicatingVarianceSwap() {
    ...
    for (auto & value : values) {
        ...
        if (error>value.tol)
            REPORT_FAILURE("value", values[i].type, value.varStrike,
                           value.nominal, value.s, value.q,
                           value.r, today, exDate, value.v, expected,
...
void VarianceSwapTest::testMCVarianceSwap() {
    ...
    for (auto & value : values) {
        Date exDate = today + Integer(value.t*365+0.5);
        Date intermDate = today + Integer(value.t1*365+0.5);
