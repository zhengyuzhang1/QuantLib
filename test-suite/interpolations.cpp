...
namespace {
    ...
    void
    checkValues(const char* type, const CubicInterpolation& cubic, I xBegin, I xEnd, J yBegin) {
    ...
    void
    check1stDerivativeValue(const char* type, const CubicInterpolation& cubic, Real x, Real value) {
    ...
    void
    check2ndDerivativeValue(const char* type, const CubicInterpolation& cubic, Real x, Real value) {
    ...
    Real multif(Real s, Real t, Real u, Real v, Real w) {
        return std::sqrt(s * std::sinh(std::log(t)) + std::exp(std::sin(u) * std::sin(3 * v)) +
...
void InterpolationTest::testSplineOnRPN15AValues() {
    ...
    const std::vector<Real> RPN15A_x = {7.99, 8.09, 8.19, 8.7, 9.2, 10.0, 12.0, 15.0, 20.0};
    const std::vector<Real> RPN15A_y = {0.0,      2.76429e-5, 4.37498e-5, 0.169183, 0.469428,
    ...
    CubicInterpolation f = CubicInterpolation(
        RPN15A_x.begin(), RPN15A_x.end(), RPN15A_y.begin(), CubicInterpolation::Spline, false,
        CubicInterpolation::SecondDerivative, 0.0, CubicInterpolation::SecondDerivative, 0.0);
    f.update();
    checkValues("Natural spline", f, RPN15A_x.begin(), RPN15A_x.end(), RPN15A_y.begin());
    check2ndDerivativeValue("Natural spline", f, *RPN15A_x.begin(), 0.0);
    check2ndDerivativeValue("Natural spline", f, *(RPN15A_x.end() - 1), 0.0);
    // poor performance
    Real x_bad = 11.0;
    ...
    f = CubicInterpolation(RPN15A_x.begin(), RPN15A_x.end(), RPN15A_y.begin(),
                           ...
                           0.0, CubicInterpolation::FirstDerivative, 0.0);
    f.update();
    checkValues("Clamped spline", f, RPN15A_x.begin(), RPN15A_x.end(), RPN15A_y.begin());
    check1stDerivativeValue("Clamped spline", f, *RPN15A_x.begin(), 0.0);
    check1stDerivativeValue("Clamped spline", f, *(RPN15A_x.end() - 1), 0.0);
    // poor performance
    interpolated = f(x_bad);
    if (interpolated < 1.0) {
    ...
    // Not-a-knot spline
    f = CubicInterpolation(RPN15A_x.begin(), RPN15A_x.end(), RPN15A_y.begin(),
                           CubicInterpolation::Spline, false, CubicInterpolation::NotAKnot,
                           Null<Real>(), CubicInterpolation::NotAKnot, Null<Real>());
    f.update();
    checkValues("Not-a-knot spline", f, RPN15A_x.begin(), RPN15A_x.end(), RPN15A_y.begin());
    checkNotAKnotCondition("Not-a-knot spline", f);
    // poor performance
    interpolated = f(x_bad);
    if (interpolated < 1.0) {
    ...
    f.update();
    checkValues("MC natural spline", f, RPN15A_x.begin(), RPN15A_x.end(), RPN15A_y.begin());
    // good performance
    interpolated = f(x_bad);
    if (interpolated > 1.0) {
    ...
    f = CubicInterpolation(RPN15A_x.begin(), RPN15A_x.end(), RPN15A_y.begin(),
                           CubicInterpolation::Spline, true, CubicInterpolation::FirstDerivative,
                           0.0, CubicInterpolation::FirstDerivative, 0.0);
    f.update();
    checkValues("MC clamped spline", f, RPN15A_x.begin(), RPN15A_x.end(), RPN15A_y.begin());
    check1stDerivativeValue("MC clamped spline", f, *RPN15A_x.begin(), 0.0);
    check1stDerivativeValue("MC clamped spline", f, *(RPN15A_x.end() - 1), 0.0);
    // good performance
    interpolated = f(x_bad);
    if (interpolated > 1.0) {
    ...
    f = CubicInterpolation(RPN15A_x.begin(), RPN15A_x.end(), RPN15A_y.begin(),
                           CubicInterpolation::Spline, true, CubicInterpolation::NotAKnot,
                           Null<Real>(), CubicInterpolation::NotAKnot, Null<Real>());
    f.update();
    checkValues("MC not-a-knot spline", f, RPN15A_x.begin(), RPN15A_x.end(), RPN15A_y.begin());
    // good performance
    interpolated = f(x_bad);
    if (interpolated > 1.0) {
        BOOST_ERROR("MC clamped spline interpolation "
...
void InterpolationTest::testSplineOnGenericValues() {
    ...
    // Natural spline
    CubicInterpolation f(generic_x.begin(), generic_x.end(), generic_y.begin(),
                         CubicInterpolation::Spline, false, CubicInterpolation::SecondDerivative,
                         generic_natural_y2[0], CubicInterpolation::SecondDerivative,
                         generic_natural_y2[n - 1]);
    f.update();
    checkValues("Natural spline", f, generic_x.begin(), generic_x.end(), generic_y.begin());
    ...
    f = CubicInterpolation(generic_x.begin(), generic_x.end(), generic_y.begin(),
                           CubicInterpolation::Spline, false, CubicInterpolation::FirstDerivative,
                           y1a, CubicInterpolation::FirstDerivative, y1b);
    f.update();
    checkValues("Clamped spline", f, generic_x.begin(), generic_x.end(), generic_y.begin());
    check1stDerivativeValue("Clamped spline", f, *generic_x.begin(), 0.0);
    check1stDerivativeValue("Clamped spline", f, *(generic_x.end() - 1), 0.0);
    x35[0] = f(3.5);
    ...
    f = CubicInterpolation(generic_x.begin(), generic_x.end(), generic_y.begin(),
                           ...
                           Null<Real>(), CubicInterpolation::NotAKnot, Null<Real>());
    f.update();
    checkValues("Not-a-knot spline", f, generic_x.begin(), generic_x.end(), generic_y.begin());
    checkNotAKnotCondition("Not-a-knot spline", f);
...
void InterpolationTest::testSimmetricEndConditions() {
    ...
    // Not-a-knot spline
    CubicInterpolation f(x.begin(), x.end(), y.begin(), CubicInterpolation::Spline, false,
                         CubicInterpolation::NotAKnot, Null<Real>(), CubicInterpolation::NotAKnot,
                         Null<Real>());
    f.update();
    checkValues("Not-a-knot spline", f, x.begin(), x.end(), y.begin());
    checkNotAKnotCondition("Not-a-knot spline", f);
...
void InterpolationTest::testNonRestrictiveHymanFilter() {
    ...
    // MC Not-a-knot spline
    CubicInterpolation f(x.begin(), x.end(), y.begin(), CubicInterpolation::Spline, true,
                         CubicInterpolation::NotAKnot, Null<Real>(), CubicInterpolation::NotAKnot,
                         Null<Real>());
    f.update();
    interpolated = f(zero);
...
void InterpolationTest::testMultiSpline() {
    ...
    Real &s = args[0] = offsets[0], &t = args[1] = offsets[1], &u = args[2] = offsets[2],
         &v = args[3] = offsets[3], &w = args[4] = offsets[4];
...
namespace {
    ...
    Integer sign(Real y1, Real y2) { return y1 == y2 ? 0 : y1 < y2 ? 1 : -1; }
...
void InterpolationTest::testFritschButland() {
    ...
    const std::vector<std::vector<Real> > y = {
        {1.0, 2.0, 1.0, 1.0, 2.0}, {1.0, 2.0, 1.0, 1.0, 1.0}, {2.0, 1.0, 0.0, 2.0, 3.0}};
...
void InterpolationTest::testSabrInterpolation() {
    ...
    for (Size i = 0; i < strikes.size(); i++) {
        Real calculatedVol = sabrVolatility(strikes[i], forward, expiry, initialAlpha, initialBeta,
    ...
    std::shared_ptr<EndCriteria> endCriteria(new EndCriteria(100000, 100, 1e-8, 1e-8, 1e-8));
...
void InterpolationTest::testKernelInterpolation2D() {
    ...
    KernelInterpolation2D kernel2DEp(xVec1.begin(), xVec1.end(), yVec1.begin(), yVec1.end(), M1,
                                     &epanechnikovKernel);
...
namespace {
    ...
    Real limCos(Real h) { return -std::cos(h); }
}
...
void InterpolationTest::testUnknownRichardsonExtrapolation() {
    ...
    const std::vector<std::pair<Real, Real> > testCases = {
        std::make_pair(1.0, 1.0),  std::make_pair(1.0, -1.0), std::make_pair(2.0, 0.25),
        std::make_pair(2.0, -1.0), std::make_pair(3.0, 2.0),  std::make_pair(3.0, -0.5),
...
namespace {
    Real f(Real h) { return std::pow(1.0 + h, 1 / h); }
}
...
void InterpolationTest::testNoArbSabrInterpolation() {
    ...
    for (Size j = 1; j < methods_.size(); ++j) { // skip simplex (gets caught in some cases)
        for (bool i : vegaWeighted) {
            for (bool k_a : isAlphaFixed) {
                for (Size k_b = 0; k_b < 1; ++k_b) { // keep beta fixed (all 4 params free is a
                                                     // problem for this kind of test)
                    for (bool k_n : isNuFixed) {
                        for (bool k_r : isRhoFixed) {
...
void InterpolationTest::testSabrSingleCases() {
    ...
    SABRInterpolation s0(strikes.begin(), strikes.end(), vols.begin(), tte, forward, Null<Real>(),
                         0.25, Null<Real>(), Null<Real>(), false, true, false, false);
    s0.update();

    if (s0.maxError() > 0.01 || s0.rmsError() > 0.01) {
        BOOST_ERROR("Sabr case #1 failed with max error (" << s0.maxError() << ") and rms error ("
                                                           << s0.rmsError()
                                                           << "), both should be < 0.01");
    }
}
    ...
    for (Size i = 0; i < 1E6; ++i) {
        ...
        QL_REQUIRE(sigmaI >= detail::NoArbSabrModel::sigmaI_min &&
                   ...
                   "sigmaI = alpha*forward^(beta-1.0) ("
                       << sigmaI << ") out of bounds, alpha=" << alpha << " beta=" << beta
                       << " forward=" << forward);
...
namespace {
    Real lagrangeTestFct(Real x) { return std::fabs(x) + 0.5 * x - x * x; }
}
...
void InterpolationTest::testLagrangeInterpolation() {
    ...
    const Real references[] = {
        -0.5000000000000000, -0.5392414024347419, -0.5591485962711904, -0.5629199661387594,
        -0.5534414777017116, -0.5333043347921566, -0.5048221831582063, -0.4700478608272949,
        -0.4307896950846587, -0.3886273460669714, -0.3449271969711449, -0.3008572908782903,
        -0.2574018141928359, -0.2153751266968088, -0.1754353382192734, -0.1380974319209344,
        -0.1037459341938971, -0.0726471311765894, -0.0449608318838433, -0.0207516779521373,
        0.0000000000000000,  0.0173877793964286,  0.0315691961126723,  0.0427562482700356,
        0.0512063534145595,  0.0572137590808174,  0.0611014067405497,  0.0632132491361394,
        0.0639070209989264,  0.0635474631523613,  0.0625000000000000,  0.0611248703983366,
        0.0597717119144768,  0.0587745984686508,  0.0584475313615655,  0.0590803836865967,
        0.0609352981268212,  0.0642435381368876,  0.0692027925097279,  0.0759749333281079,
        0.0846842273010179,  0.0954160004849021,  0.1082157563897290,  0.1230887474699003,
        0.1400000000000001,  0.1588747923353829,  0.1795995865576031,  0.2020234135046815,
        0.2259597111862140,  0.2511886165833182,  0.2774597108334206,  0.3044952177998833,
        0.3319936560264689,  0.3596339440766487,  0.3870799592577457,  0.4139855497299214,
        0.4400000000000001,  0.4647739498001331,  0.4879657663513030,  0.5092483700116673,
        0.5283165133097421,  0.5448945133624253,  0.5587444376778583,  0.5696747433431296,
        0.5775493695968156,  0.5822972837863635,  0.5839224807103117,  0.5825144353453510,
        0.5782590089582251,  0.5714498086024714,  0.5625000000000000,  0.5519545738075141,
        0.5405030652677689,  0.5289927272456703,  0.5184421566492137,  0.5100553742352614,
        0.5052363578001620,  0.5056040287552059,  0.5130076920869246};

    const Real tol = 50 * QL_EPSILON;
    for (Size i = 0; i < 79; ++i) {
        const Real xx = -1.0 + i * 0.025;
        const Real calculated = interpl(xx);
        if (boost::math::isnan(calculated) || std::fabs(references[i] - calculated) > tol) {
...
void InterpolationTest::testLagrangeInterpolationAtSupportPoint() {
    ...
    for (Size i = 1; i < n - 1; ++i) {
        for (Real z = x[i] - 100 * QL_EPSILON; z < x[i] + 100 * QL_EPSILON; z += 2 * QL_EPSILON) {
            ...
            if (boost::math::isnan(calculated) || std::fabs(expected - calculated) > relTol) {
...
void InterpolationTest::testLagrangeInterpolationDerivative() {
    ...
    for (Real x = -1.0; x <= 0.9; x += 0.01) {
        ...
        if (boost::math::isnan(calculated) || std::fabs(expected - calculated) > 25 * eps) {
...
void InterpolationTest::testLagrangeInterpolationOnChebyshevPoints() {
    BOOST_TEST_MESSAGE("Testing Lagrange interpolation on Chebyshev points...");
    ...
    for (Real x = -1.0; x <= 1.0; x += 0.03) {
        ...
        const Real expectedDeriv =
            std::exp(x) * (std::cos(x) + std::sin(x)) / square<Real>()(std::cos(x));
...
void InterpolationTest::testBSplines() {
    ...
    referenceValues += std::make_tuple(0, -0.95, 9.5238095238e-04),
        std::make_tuple(0, -0.01, 0.37337142857), std::make_tuple(0, 0.49, 0.84575238095),
        std::make_tuple(0, 1.21, 0.0), std::make_tuple(1, 1.49, 0.562987654321),
        std::make_tuple(1, 1.59, 0.490888888889), std::make_tuple(2, 1.99, 0.62429409171),
        std::make_tuple(3, 1.19, 0.0), std::make_tuple(3, 1.99, 0.12382936508),
    ...
    for (auto& referenceValue : referenceValues) {
        ...
        if (boost::math::isnan(calculated) || std::fabs(calculated - expected) > tol) {
...
void InterpolationTest::testBackwardFlatOnSinglePoint() {
    ...
    const Interpolation impl(
        BackwardFlat().interpolate(knots.begin(), knots.end(), values.begin()));
